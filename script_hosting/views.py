...
def run_script(request, file_id):
    if request.method == 'POST':
        script_file = get_object_or_404(File, id=file_id, owner=request.user)

        try:
            # 这里假设脚本是Python脚本，您可以根据需要调整
            output = subprocess.check_output(['python', script_file.get_file_path()], stderr=subprocess.STDOUT)
            return JsonResponse({'status': 'success', 'output': output.decode()})
        except subprocess.CalledProcessError as e:
            return JsonResponse({'status': 'error', 'output': e.output.decode()})
...
def index(request):
    if request.method == "GET":
        Folder.create_script()
        return redirect(reverse("script_hosting:folder_show", kwargs={"path":"script"}))
